...
const FuturisticModal = ({ open, onClose, title, children, actions = [] }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      {/* Modal Card */}
      <div className="relative z-[110] w-[95%] max-w-xl rounded-xl overflow-hidden border border-cyan-800/40 bg-gray-900/70 shadow-2xl">
        <div
          ...
          <div className="relative px-5 py-4 bg-black/40 border-t border-cyan-900/40 flex items-center justify-end gap-3">
            {actions.map((a, idx) => (
              <button
                ...
                disabled={a.disabled}
                className={
                  `px-4 py-2 rounded-md font-['Orbitron'] text-sm transition-colors cursor-pointer flex items-center gap-2 ` +
                  (a.variant === 'danger'
