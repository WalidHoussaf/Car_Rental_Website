    ...
    <div className="bg-gradient-to-br from-gray-900/50 to-black/50 border border-gray-800 rounded-xl p-6 h-fit">
      ...
      <div className="space-y-4">
        <HighlightItem 
          ...
          description={`Accelerate from 0-60 mph in just ${car.specifications.acceleration?.split(' ')[0] || 'record'} seconds with ${car.specifications.horsepower} of raw power.`} 
        />
        
        <HighlightItem 
          title="Cutting-Edge Technology" 
          description="Experience the latest in automotive innovation with advanced driver assistance systems and connectivity features." 
        />
        ...
        {car.features && car.features.length > 0 && (
          car.features.slice(0, 2).map((feature, index) => (
            <HighlightItem 
              key={index}
              title={feature} 
              description="Premium quality and performance in every detail." 
            />
          ))
        )}
      </div>
      ...
      <div className="mt-8 p-4 bg-blue-900/20 rounded-lg border border-blue-500/30">
        <div className="flex items-start">
          ...
          <div className="ml-3">
            <h4 className="text-sm font-medium text-cyan-400 font-['Orbitron']">Limited Availability</h4>
            <p className="mt-1 text-2xs text-gray-300 font-['Rationale']">This vehicle is in high demand. We recommend booking in advance to secure your experience.</p>
          </div>
        </div>
      </div>
    </div>
